...
export default function AppControlCenterPage() {
  ...
  const appId = params.appId || "mindshot-golf";
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/control-center/${appId}`, {
        cache: "no-store",
      });
  ...
  const riskCount =
    data.embr.placeholderRiskCount +
    data.embr.boundaryRiskCount +
    data.embr.inventedDataRiskCount;
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-8 text-white">
      <div className="mx-auto max-w-6xl space-y-8">
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-bold text-emerald-300">
                Live Embr data connected
              </p>
              <p className="mt-1 text-sm text-emerald-100/80">
                App health, Embr usage, quality score, and last checked time are
                coming from the live Embr backend.
              </p>
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <Panel title="App Details" badge="Live">
          ...
          <Panel title="Business Snapshot" badge="Pending">
            <div className="grid gap-3 sm:grid-cols-2">
            ...
            <p className="mt-4 text-xs text-slate-500">
              These metrics are pending until MindShot user and subscription data
              sources are connected.
            </p>
          </Panel>
        </section>

        <Panel title="Embr Intelligence" badge="Live">
          ...
          <div className="mt-5 rounded-2xl border border-slate-800 bg-slate-950 p-4">
            ...
            <p className="mt-2 text-xs text-slate-500">
              Question clustering and user insight summaries are coming next.
            </p>
          </div>
        </Panel>
        ...
        <p className="text-center text-xs text-slate-600">
          Technical server details are available only in the internal Embr
          Operator Dashboard.
        </p>
      </div>
    </main>
...
function StatusCard({ label, value }: { label: string; value: string }) {
  ...
  const pending = value.toLowerCase().includes("pending");
      ...
      <div className="mt-3 flex items-center gap-3">
        <span
          className={`h-3 w-3 rounded-full ${
            healthy ? "bg-emerald-400" : pending ? "bg-yellow-400" : "bg-red-400"
          }`}
        />
